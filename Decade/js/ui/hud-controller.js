...
const HudController = (function () {
	...
	function init() {
		...
		_elements.levelScore = document.getElementById("hud-level-score");
	...
	function setLevel(level) {
		if (_elements.level) {
			const formatted =
				level === 4.5 ? "4B" : level === 7.5 ? "7B" : level === 10.5 ? "10B" : String(level).padStart(2, "0");
	...
	/**
	 * Update the level score display (current level only)
	 * @param {number} score - Level score value
	 */
	function setLevelScore(score) {
		if (!_elements.levelScore) return;
		_elements.levelScore.textContent = String(score).padStart(4, "0");
		_currentValues.levelScore = score;
		_renderCollectiblesHUD();
	}

	/**
	 * Update the cumulative score display (all levels)
	 * @param {number} score - Total score value
	 ...
	 * @param {Object} data - Object with level, year, score, levelScore properties
	 */
	function update(data) {
		if (data.level !== undefined) setLevel(data.level);
		if (data.year !== undefined) setYear(data.year);
		if (data.levelScore !== undefined) setLevelScore(data.levelScore);
	...
	function _handleScoreChange(data) {
		// total = cumulative (all levels), levelScore = current level only
		if (data.levelScore !== undefined) setLevelScore(data.levelScore);
		if (data.total !== undefined) setScore(data.total, true);
	}
	...
	function _handleLevelLoad(data) {
		...
		// Keep HUD scores in sync (total = cumulative, levelScore = current level)
		if (typeof StateManager !== "undefined") {
			if (StateManager.getFinalScore) setScore(StateManager.getFinalScore(), false);
			const ld = StateManager.getLevelData ? StateManager.getLevelData() : {};
			setLevelScore(ld.score != null ? ld.score : 0);
		}
		_renderCollectiblesHUD();
	}
	...
	function _handleCollectibleCollected(payload) {
		...
		const icon =
			payload.icon ||
			(typeof StateManager !== "undefined" && StateManager.getCollectibleIconForEraKey
				? StateManager.getCollectibleIconForEraKey(payload.eraKey)
				: "");
		const total =
			payload.total ||
			(typeof StateManager !== "undefined" && StateManager.getCollectibleTotalForEraKey
